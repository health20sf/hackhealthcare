...
(when-not (exists? js/ReactDOMServer)
  (let [react-dom-server (nodejs/require "react-dom/server")]
    ...
    (when-not (exists? js/ReactDOMServer)
      (set! js/ReactDOMServer react-dom-server))))

;; required for recharts only (still relevant?)
(when-not (exists? js/goog.global.setTimeout)
  (assert js/setTimeout)
  (reset! js/goog.global.setTimeout js/setTimeout))
...
;; really needed?
#_
(def react-dom (nodejs/require "react-dom"))
#_
(when react-dom
  ;; work-arounds "constant React assigned a value more than once."
  (aset js/global "react-dom" react-dom))

;; Can't yet set window before loading react.js as it uses it to test for canUseDom
;; wait until DOM implementaiton is more complete? WORKS NOW MARCH 2017,
...
;; redo in case node prototypes are cached:
...
(defn prototype-instantiate [p] ;; not used??
  (let [inst (fn [])]
...
(defn override-prototype [obj prototype & [type]]
  (set! (.-prototype obj)
     (js-merge
      (if type (type-prototype type) #js{})
      prototype))
  #_ ;; seems no longer needed and causing missing prototype issue on advanced compilation
  (set! (.-__proto__ obj)
     (merge
      (if type (proto type) #{})
      prototype))
